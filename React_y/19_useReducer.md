...
: useState의 대안으로 사용, 복잡한 상태 업데이트 로직을 명확하게 구조화하고 관리하기 위한 Hook <br />

> - const [state, dispatch] = useReducer(reducer, initialState); <br />
>   : reducer 함수는 우리가 직접 만들어야 하는 상태 로직 함수. (일반적으로 switch문 구조) <br />

> - dispatch({ type: "add", payload: { text: "React 공부하기" } }); <br />
>   : type 어떤 동작을 할지 나타내는 문자열(필수), payload 상태 변경에 필요한 데이터(선택) <br />
>   : 화면 표시와 직접 관련이 없는 추가 정보나 부가 데이터는 meta 속성에 담아 함께 전달가능 <br />
